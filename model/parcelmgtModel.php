...
function addParcel($username, $parcelName, $percelType, $percelFrom, $percelTo, $rname, $rnumber)
{
    ...
    if (!$row_id || empty($row_id["parcelID"])) {
        $new_unique_id = "PCL1";
    } else {
        ...
        do {
            ...
            $check_query = "SELECT parcelID FROM parcelmanagement WHERE parcelID = '$new_unique_id'";
...
{
    $con = getConnection();
    $sql = "UPDATE parcelmanagement set parcelName='{$NewParcelName}',rname='{$newRname}',rnumber='{$newrNumber}',ParcelStatus='{$parcelStatus}' where parcelID = '{$parcelID}'";
    $result = mysqli_query($con, $sql);
    $update = [];
    if($result==true){
        header('location: ../view/parcelmanagementPanel.php');
    }
    return $update;
}
